...
const G=new Function(src+';return {WAVES,TOWERS,ARCH,AGES,C}')();
let wt='| w | type | n | hp | spd | armor | regen | cap | bounty |\n|---|---|---|---|---|---|---|---|---|\n';
G.WAVES.forEach((r,i)=>{wt+='| '+(i+1)+' | '+r.name+(r.rush?' (rush)':'')+' | '+r.n+' | '+r.hp+' | '+r.spd.toFixed(2)+' | '+r.ar+' | '+r.rg+' | '+(r.cap||'—')+' | '+r.bty+' |\n'});
let pal='| age | ground | path | accent | bpm | root Hz | osc |\n|---|---|---|---|---|---|---|\n';
G.AGES.forEach(a=>{pal+='| '+a[0]+' | `'+a[1]+'` | `'+a[2]+'` | `'+a[3]+'` | '+a[4]+' | '+a[5]+' | '+a[6]+' |\n'});
const D=`# EPOCH — Design Spec (source of truth)
...
`;
fs.writeFileSync(__dirname+'/DESIGN.md',D);
console.log('DESIGN.md written, '+D.split('\n').length+' lines');
